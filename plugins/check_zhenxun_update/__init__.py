...
usage：
    ...
    指令：
        ...
        重启
""".strip()
__plugin_des__ = "就算是梦落也会成长的"
__plugin_cmd__ = ["检查更新梦落", "重启"]
...
update_zhenxun = on_command("检查更新梦落", permission=SUPERUSER, priority=1, block=True)
...
async def _(bot: Bot, event: MessageEvent, state: T_State):
    try:
        ...
        if error:
            logger.error(f"更新梦落未知错误 {error}")
            await bot.send_private_msg(
                user_id=int(list(bot.config.superusers)[0]), message=f"更新梦落未知错误 {error}"
            )
    except Exception as e:
        logger.error(f"更新梦落未知错误 {type(e)}：{e}")
        await bot.send_private_msg(
            user_id=int(list(bot.config.superusers)[0]),
            message=f"更新梦落未知错误 {type(e)}：{e}",
        )
    else:
        if code == 200:
            await bot.send_private_msg(
                user_id=int(list(bot.config.superusers)[0]), message=f"更新完毕，请重启梦落...."
            )
...
@restart.got("flag", prompt="确定是否重启梦落？（重启失败咱们将失去联系，请谨慎！）")
async def _(bot: Bot, event: MessageEvent, state: T_State):
    flag = state["flag"]
    if flag.lower() in ["true", "是", "好", "确定", "确定是"]:
        await restart.send("开始重启梦落..请稍等...")
        open("is_restart", "w")

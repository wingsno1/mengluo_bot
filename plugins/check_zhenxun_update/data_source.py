...
async def check_update(bot: Bot) -> 'int, str':
    ...
    if data:
        ...
        if _version != latest_version:
            ...
            logger.info(f"检测梦落已更新，当前版本：{_version}，最新版本：{latest_version}")
            await bot.send_private_msg(
                user_id=int(list(bot.config.superusers)[0]),
                message=f"检测梦落已更新，当前版本：{_version}，最新版本：{latest_version}\n" f"开始更新.....",
            )
            logger.info(f"开始下载梦落最新版文件....")
            if await download_latest_file(tar_gz_url):
                logger.info("下载梦落最新版文件完成....")
                error = await asyncio.get_event_loop().run_in_executor(
                    None, _file_handle, latest_version
                )
                if error:
                    return 998, error
                logger.info("梦落更新完毕，清理文件完成....")
                logger.info("开始获取梦落更新日志.....")
                ...
                await bot.send_private_msg(
                    user_id=int(list(bot.config.superusers)[0]),
                    message=Message(f"梦落更新完成，版本：{_version} -> {latest_version}\n"
                ...
                logger.warning(f"下载梦落最新版本失败...版本号：{latest_version}")
                await bot.send_private_msg(
                    user_id=int(list(bot.config.superusers)[0]),
                    message=f"下载梦落最新版本失败...版本号：{latest_version}.",
                )
        else:
            logger.info(f"自动获取梦落版本成功：{latest_version}，当前版本为最新版，无需更新...")
            await bot.send_private_msg(
                user_id=int(list(bot.config.superusers)[0]),
                message=f"自动获取梦落版本成功：{latest_version}，当前版本为最新版，无需更新...",
            )
    else:
        logger.warning("自动获取梦落版本失败....")
        await bot.send_private_msg(
            user_id=int(list(bot.config.superusers)[0]), message=f"自动获取梦落版本失败...."
        )
    return 999, ''
